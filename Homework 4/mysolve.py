...
def mysolve(A,b, **kwargs):
    global SOLVER
    if SOLVER == 'LU':
        return True, LUsolve(A,b, **kwargs)
    if SOLVER == 'LUcsr':
        return True, LUcsrsolve(A,b, **kwargs)
    elif SOLVER=='special':
        x = LUcsrsolve(A,b, **kwargs)
        print("precision =",np.linalg.norm(A@x - b,ord=2)/np.linalg.norm(b,ord=2))
        return True, x
    else:
        return False, None
...
def plot_matrix(A, name):
    plt.imshow(np.clip(A*A, 0, 1), cmap='Purples')
    # plt.savefig('res/matrix/Standard/%s' %(name))

def save_matrix(A, filename):
    matrix = np.matrix(A[:20,:20])
    with open("res/matrix/%s"%filename,'wb') as fd:
        for line in matrix:
...
if __name__=='__main__':
    ...
    print('Solution de numpy :\n', np.linalg.solve(A,b))
    print('Solution de LUcsrsolve :\n', LUcsrsolve(A,b,**{'RCMK':True}))
